...
for(let i=0;i<allCells.length;i++){
    ...
    allCells[i].addEventListener("blur",function(e){   //blur help ki jab ham dusre parjayehege tab ye value ko change kr dega
        lastSelectedCell = e.target;
        let cellValue = e.target.textContent;
        let {rowId,colId} = getRowIdColIdFromElement(e.target);
        let cellObject = db[rowId][colId];
        if(cellObject.value == cellValue){     //bina kuch leekhe gye too not change only return
            return;
        }
